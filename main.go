...
func LatestVersions(releases []*semver.Version, minVersion *semver.Version) []*semver.Version {
	...
	length := len(releases)
	major := releases[length-1].Slice()[0]
	minor := releases[length-1].Slice()[1]
	versionSlice = append(versionSlice, releases[length-1])

	for i := length-2; i >= 0; i-- {
		if releases[i].LessThan(*minVersion) {
			break;
		}
		tmp_major := releases[i].Slice()[0]
		tmp_minor := releases[i].Slice()[1]
